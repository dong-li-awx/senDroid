...
int binder_parse(struct binder_io *bio, uint32_t *ptr, uint32_t size, binder_handler func)
{
    int r = 1;
                ...
                void *target, uint32_t code)
{
    ...
    for (;;) {
        ...
        res = binder_parse(reply, readbuf, bwr.read_consumed, 0);
        if (res == 0) return 0;
        if (res < 0) goto fail;
    }
...
void binder_loop(struct binder_state *bs, binder_handler func)
{
    ...
    for (;;) {
        ...
        res = binder_parse(0, readbuf, bwr.read_consumed, func);
        if (res == 0) {
...
{
    //LOGD("data: %x, offs: %x, data_size: %d, offs_size: %d", txn->data, txn->offs, txn->data_size, txn->offs_size);
